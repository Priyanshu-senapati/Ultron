...
### Full Phase-1 stack (six processes)
...
# Terminal 3 — Module D Memory Engine (SQLite persistence + learned priors)
...
# Terminal 6 — watch everything go past, including remote `ghost:*` events
python python\bridge_test.py --filter insight_snapshot,visual_label,window_changed,ghost:insight_snapshot,ghost:tension_changed
```
...
Next: **Phase 2 — agents + tooling** (no more Phase 1 modules pending!).
