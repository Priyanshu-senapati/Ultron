...
pub struct Config {
    ...
    /// Phase 1, Module D — Memory Engine sidecar. `#[serde(default)]`
    /// so configs from earlier turns upgrade transparently.
    #[serde(default)]
...
/// Phase 1, Module D — Memory Engine sidecar. The sidecar reads these
/// values; the core daemon only carries them so they live in the same
/// canonical config file.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MemoryConfig {
    /// How often the prior-learner runs and publishes a
    /// `productivity_prior_update`, in seconds. Build prompt asks for
    /// "every 10–15 minutes"; default 600 (10 min).
    pub prior_tick_secs: u64,
    /// Append a "prior_published" note to the Quantum Log every Nth
    /// tick. Default 6 → once per hour with the 10-min tick.
    pub log_every_n_ticks: u64,
    /// How often pattern detection runs and publishes a
    /// `patterns_update`, in seconds. Patterns evolve slowly (need days
    /// of history before signal is meaningful), so a slower cadence
    /// than the prior tick is right. Default 3600 (1 hour).
    #[serde(default = "default_memory_pattern_tick_secs")]
    pub pattern_tick_secs: u64,
    /// Override the memory database path. Empty string ⇒ use the default
    /// `<data_dir>/memory.db`. Kept as a string (not `Option<PathBuf>`)
    /// because TOML is friendlier with strings and the empty-string
    /// sentinel matches how `bridge.token` does it.
    #[serde(default)]
    pub db_path: String,
}

fn default_memory_pattern_tick_secs() -> u64 {
    60 * 60
}
...
impl Default for MemoryConfig {
    fn default() -> Self {
        Self {
            prior_tick_secs: 10 * 60,
            log_every_n_ticks: 6,
            pattern_tick_secs: 60 * 60,
            db_path: String::new(),
        }
    }
}
