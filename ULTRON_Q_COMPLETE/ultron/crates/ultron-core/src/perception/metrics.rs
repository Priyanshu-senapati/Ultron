...
use ultron_types::{clamp01, InputMetrics, InputSignal, KeyCategory, UltronEvent};
...
struct Inner {
    ...
    last_input_ms: i64,
    /// WPM samples over the last 30 minutes — `(ts_unix_ms, wpm)`.
    /// Pushed on every `tick()`. Used by `wpm_slope_per_hour` to detect
    /// declining productivity (Fix 1 of the Module-O prep).
    wpm_history: VecDeque<(i64, f32)>,
}

/// How far back we keep WPM samples for trend analysis: 30 minutes.
const WPM_HISTORY_MS: i64 = 30 * 60 * 1000;
...
impl InputMetricsAggregator {
    ...
    pub fn tick(&self) -> InputMetrics {
        ...
        // Push current WPM into the trend history, prune to the 30-min window,
        // then run a linear regression for slope.
        g.wpm_history.push_back((now_ms, wpm));
        let wpm_cutoff = now_ms - WPM_HISTORY_MS;
        while let Some(&(t, _)) = g.wpm_history.front() {
            if t < wpm_cutoff {
                g.wpm_history.pop_front();
            } else {
                break;
            }
        }
        let wpm_slope_per_hour = wpm_slope_per_hour_inner(&g.wpm_history);
    ...
    /// Public accessor for WPM trend slope. Useful for ad-hoc inspection
    /// (tests, debug RPCs). Production code reads it via `tick()` →
    /// `InputMetrics.wpm_slope_per_hour`.
    pub fn wpm_slope_per_hour(&self) -> f32 {
        let g = self.inner.lock();
        wpm_slope_per_hour_inner(&g.wpm_history)
    }
...
/// Least-squares slope of WPM vs. time, expressed in **WPM per hour**.
///
/// Returns `0.0` if the history has fewer than 3 points, or if the time
/// span is degenerate (all samples bunched at the same millisecond — which
/// would cause a div-by-zero in the closed-form slope).
fn wpm_slope_per_hour_inner(history: &VecDeque<(i64, f32)>) -> f32 {
    if history.len() < 3 {
        return 0.0;
    }
    // Use the first timestamp as origin to keep the numbers in a sane range.
    let t0 = history.front().unwrap().0;
    let n = history.len() as f64;
    let mut sx = 0.0f64; // Σ x  (x = seconds since t0)
    let mut sy = 0.0f64; // Σ y
    let mut sxx = 0.0f64; // Σ x²
    let mut sxy = 0.0f64; // Σ x y
    for &(t, w) in history.iter() {
    ...
    // Slope in (WPM / second). Convert to WPM / hour.
    let slope_per_sec = (n * sxy - sx * sy) / denom;
    let slope_per_hour = slope_per_sec * 3600.0;
    if slope_per_hour.is_finite() {
        slope_per_hour as f32
    } else {
        0.0
    }
}
...
mod tests {
    ...
    #[test]
    fn wpm_slope_zero_with_few_points() {
        // Synthesise a history with only 2 entries — slope must be 0.0.
        let mut h = std::collections::VecDeque::new();
        h.push_back((0i64, 50.0f32));
        h.push_back((60_000i64, 60.0f32));
        assert_eq!(super::wpm_slope_per_hour_inner(&h), 0.0);
    }

    #[test]
    fn wpm_slope_negative_when_declining() {
        // 4 samples spread over 10 min, dropping from 80 → 50 WPM linearly.
        // Slope per second = -30 / 600 = -0.05 wpm/s → -180 wpm/hour.
        let mut h = std::collections::VecDeque::new();
        for i in 0..=10 {
            let t = i as i64 * 60_000;
            let w = 80.0 - 3.0 * i as f32;
            h.push_back((t, w));
        }
        let slope = super::wpm_slope_per_hour_inner(&h);
        assert!(slope < -150.0 && slope > -210.0, "slope was {slope}");
    }

    #[test]
    fn wpm_slope_positive_when_rising() {
        let mut h = std::collections::VecDeque::new();
        for i in 0..=10 {
            let t = i as i64 * 60_000;
            let w = 30.0 + 3.0 * i as f32;
            h.push_back((t, w));
        }
        let slope = super::wpm_slope_per_hour_inner(&h);
        assert!(slope > 150.0, "slope was {slope}");
    }
}
