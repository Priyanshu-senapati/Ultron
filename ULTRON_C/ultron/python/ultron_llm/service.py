...
class LLMService:
    ...
    async def ask(
        ...
        # Visual question? Route through vision model.
    ...
    def _max_tokens(self, mode: str = "default") -> int:
        ...
        old 512-token cap was clipping replies mid-sentence.
        """
        if mode == "voice":
            return 1024
    ...
    async def _visual_answer(self, prompt: str, mode: str) -> Optional[str]:
        """Capture screen + ask LLaVA. Returns None on failure (fall back to text)."""
        import asyncio
        try:
