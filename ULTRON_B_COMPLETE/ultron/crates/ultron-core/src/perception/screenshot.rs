...
impl Screenshotter {
    ...
    /// Spawn the bus listener that reacts to `request_screenshot` custom
    /// events (Fix 3 of the Module-O prep). The tension tracker fires this
    /// on transitions into the `Spiked` band so the captured frame can be
    /// fed to Module O's visual context signal.
    ///
    /// Other future producers (an HUD button, the agent router from Phase 2)
    /// can fire the same event and get the same one-shot capture.
    pub fn start_listener(&self) -> tokio::task::JoinHandle<()> {
        let me = self.clone();
        let mut rx = self.bus.subscribe();
        tokio::spawn(async move {
            info!("screenshot request listener started");
            loop {
                if me.stop.load(Ordering::SeqCst) {
                    break;
                }
                let ev = match rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        warn!(
                            dropped = n,
                            "screenshot listener lagged; some events skipped"
                        );
                        continue;
                    }
                    Err(tokio::sync::broadcast::error::RecvError::Closed) => break,
                };
                if let UltronEvent::Custom(env) = &ev {
                    if env.kind == "request_screenshot" {
                        // Reason is optional; default to OnDemand.
                        let reason = env
                            .payload
                            .get("reason")
                            .and_then(|v| v.as_str())
                            .and_then(|s| match s {
                                "high_tension" => Some(ScreenshotReason::HighTension),
                                "window_change" => Some(ScreenshotReason::WindowChange),
                                "periodic" => Some(ScreenshotReason::Periodic),
                                _ => Some(ScreenshotReason::OnDemand),
                            })
                            .unwrap_or(ScreenshotReason::OnDemand);
                        if let Err(e) = me.capture_now(reason).await {
                            warn!("requested screenshot failed: {e:?}");
                        }
                    }
                }
            }
            info!("screenshot request listener stopped");
        })
    }
...
fn capture_to_dir(dir: &Path) -> anyhow::Result<CaptureResult> {
    ...
    // Fix 7 — virtual-screen metrics span every monitor in the desktop's
    // global coordinate space (origin may be negative if a monitor sits to
    // the left of the primary). Using these instead of `SM_CXSCREEN` /
    // `SM_CYSCREEN` captures the full multi-monitor layout in one blit.
    ...
    unsafe {
        ...
        if width <= 0 || height <= 0 {
            anyhow::bail!("invalid virtual screen dimensions: {width}x{height}");
        }

        // Source DC = the whole desktop (HWND(NULL) → screen DC, which
        // spans the entire virtual desktop on a multi-monitor system).
        ...
        // Blit the virtual desktop into the memory bitmap. Source origin is
        // (x, y), not (0, 0), because monitors can sit at negative offsets.
