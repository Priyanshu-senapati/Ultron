...
def fmt_insight_snapshot(p: dict) -> str:
    cl = p.get("cognitive_load", 0)
    bar = "#" * int(cl * 20)
    bar = bar.ljust(20)
    fatigue = "  FATIGUE" if p.get("fatigue_flag") else ""
    return (
        f"insight    "
        f"cl={cl:.2f} [{bar}] "
        f"t={p.get('tension', 0):.2f} "
        f"focus={p.get('focus_score', 0):.2f} "
        f"cad={p.get('cadence_band','?'):<8} "
        f"app={p.get('focus_app','?')[:20]:<20} "
        f"phase={p.get('circadian_phase','?'):<12}"
        f"{fatigue}"
    )


def fmt_visual_label(p: dict) -> str:
    return f"visual     '{p.get('label','?')}' ts={p.get('screenshot_ts','-')}"


def fmt_request_screenshot(p: dict) -> str:
    return f"request    screenshot reason={p.get('reason','?')}"


def fmt_productivity_prior(p: dict) -> str:
    priors = p.get("priors") or []
    learned_hours = sum(1 for x in priors if x is not None)
    return (
        f"prior      learned_hours={learned_hours}/24 "
        f"schema_v={p.get('schema_version','?')}"
    )


def fmt_patterns_update(p: dict) -> str:
    patterns = p.get("patterns") or []
    if not patterns:
        return f"patterns   none detected yet (schema_v={p.get('schema_version','?')})"
    # Show a one-line preview of up to 3 patterns; if more, indicate.
    head = patterns[:3]
    parts = [
        f"{x.get('kind','?')}[{x.get('confidence',0):.2f}]: {x.get('summary','?')[:60]}"
        for x in head
    ]
    suffix = f"  (+{len(patterns) - 3} more)" if len(patterns) > 3 else ""
    return f"patterns   {len(patterns)} found · " + " | ".join(parts) + suffix


def fmt_voice_transcript(p: dict) -> str:
    text = p.get("text", "")
    if len(text) > 60:
        text = text[:57] + "..."
    return (
        ...
        f"dur={p.get('duration_secs', 0):.1f}s "
        f"via={p.get('activation', '?')}"
    )


def fmt_voice_state_changed(p: dict) -> str:
    return (
        f"voice      {p.get('prev_state', '?'):<10} → {p.get('state', '?'):<10} "
        f"via={p.get('activation', '-')}"
    )


def fmt_llm_response(p: dict) -> str:
    text = p.get("text", "")
    if len(text) > 50:
        text = text[:47] + "..."
    err = "  [ERROR]" if p.get("error") else ""
    return (
        f"llm-resp   "
        f"\"{text}\" "
        f"shard={p.get('shard', '?')} "
        f"len={p.get('response_len', 0)}"
        f"{err}"
    )
...
FORMATTERS = {
    ...
    "insight_snapshot": fmt_insight_snapshot,
    "visual_label": fmt_visual_label,
    "request_screenshot": fmt_request_screenshot,
    "productivity_prior_update": fmt_productivity_prior,
    "patterns_update": fmt_patterns_update,
    "voice_transcript": fmt_voice_transcript,
    "voice_state_changed": fmt_voice_state_changed,
    "llm_response": fmt_llm_response,
}


def pretty(kind: str, payload: dict) -> str:
    # Ghost-prefixed events (from Module Q) — route to the same
    # formatter as the local equivalent, prefix the output for clarity.
    if kind.startswith("ghost:"):
        inner_kind = kind[len("ghost:"):]
        inner = FORMATTERS.get(inner_kind)
        rendered = inner(payload) if inner else f"{inner_kind}: {payload}"
        return f"\033[35m[GHOST]\033[0m {rendered}"
