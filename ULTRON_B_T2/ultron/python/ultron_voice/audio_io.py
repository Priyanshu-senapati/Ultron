...
# Type alias: level callback receives the chunk's peak amplitude in [0, 1].
# Called from the sounddevice executor thread, so implementations must be
# thread-safe (typically by scheduling onto the asyncio loop).
...
class AudioRecorder:
    ...
    def __init__(
        ...
        # Fire the level callback at most ~10Hz to avoid flooding the bus.
        # At 16kHz / 512-sample chunks that's every 3 chunks (~96ms).
        self._level_every_n_chunks = max(1, (sample_rate // 10) // CHUNK_SAMPLES)
    ...
    def _record_sync(self) -> np.ndarray:
        ...
        try:
            with sd.InputStream(
                ...
                for i in range(max_chunks):
                    ...
                    # Publish a downsampled peak so the HUD can render a
                    # live mic meter. Cheap (~one np.max per ~100ms).
                    if self.level_callback is not None and (i % self._level_every_n_chunks == 0):
                        try:
                            self.level_callback(float(np.max(np.abs(mono))))
                        except Exception as exc:
                            logger.debug("level_callback failed (ignored): %s", exc)
        ...
        # Auto-amplify quiet mics (Windows laptops with low default gain).
        # Import here to avoid a circular import at module load.
        from .wake_word import amplify_to_peak
        return amplify_to_peak(audio)
